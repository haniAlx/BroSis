...
const Vehicle = ( {SingleUser = 'false', id}) => {
  ...
  const SingleUsers =`${mainAPI}/Api/Admin/All/CompanyVehicleOwner/${id}`;
  ...
  const getAllVehicle = async () => {
    try {
      setLoading(true);
      const res = await fetch(SingleUser=='true' ? SingleUsers : apiAllVehicle, options);
      ...
      if (data && res.ok) {
        console.log(data);
        setAllVehicle(SingleUser=='true' ? data.ownerINF.vehicles:data.vehiclesINF);
        setTableData(SingleUser=='true' ? data.ownerINF.vehicles:data.vehiclesINF);
      }
      if (res.status == 400) {
  ...
  const handleAssign = (item) => {
    navigate(`/vehicle/${item.driverName ? 'changeDriver' : 'assignDriver'}/${item.ownerID}/${item.plateNumber}`);
  };
