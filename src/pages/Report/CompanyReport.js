...
const DriversReport = () => {
  ...
  const column = [
    ...
    {
      name: "TotalVehicle",
      selector: (row) => row.totalVehicles,
      field: "totalVehicles",
    },
    {
...
console.log(records)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const ownerRes = await fetch(`${mainAPI}/Api/Admin/All/VehicleOwners`, {
          headers,
        });
        ...
        if (ownerData && ownerRes.ok) {
          ...
          setRecords(ownerData.vehicleOwnerINF);
          setFilterRecords(ownerData.vehicleOwnerINF);
        }
  ...
  const handleExcelDownload = () => {
    ...
    saveAs(excelBlob, "CompanyReport.xlsx");
  };
  return (
            ...
            {records.length > 0 ? (
              records.map((item, index) => (
                <tr className="" key={index}>
                  ...
                  <td>{item.totalDrivers == 0 ? 0: item.totalDrivers }- {item.totalVehicles}</td>
                  <td>{item.revenue}</td>
