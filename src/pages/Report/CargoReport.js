...
const CargoReport = ({ currentPageCargo, inputs }) => {
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.autoTable({
      head: [["Id", "Name", "Business", "Revenue"]],
      body: inputs.map((item) => [
        item.id,
        item.ownerName,
        item.businessINF.businessName,
        item.revenue,
      ]),
    });
  ...
  const exportToExcel = () => {
    const data = inputs.map((item) => ({
      Id: item.id,
      Name: item.ownerName,
      Business: item.businessINF.businessName,
  ...
  const totalRevenue = inputs.reduce(
    (total, input) => total + input.revenue,
    0
  );
    ...
    <div className="tableReports">
      <div className="table-containers" id="report-Tables">
        <table className="data-tables">
          ...
          <tbody>
            {currentPageCargo.map((item, index) => (
